    ...
    <div class="flex-1 h-full max-w-5xl mx-auto overflow-hidden bg-white rounded-lg shadow-xl dark:bg-gray-800">
        <div class="flex flex-col overflow-y-auto md:flex-row">
            ...
            <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                <form class="w-full flex flex-col gap-y-2" method="post" action="{{ route('edit_profile.update',auth()->user()->id) }}" x-data="{whatsapp:false,phone:'{{ old('phone',auth()->user()->phone) }}'}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
